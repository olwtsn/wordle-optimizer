...
class WordleSolver:
    ...
    def find_best_starter(self, sample_size=20):
        ...
        if len(remaining_answers) > sample_size:
            remaining_answers = random.sample(remaining_answers, 
sample_size)

        self.entropy_scores = {}
        for guess in remaining_answers[:10]:  # Test first 10
            self.entropy_scores[guess] = self.calculate_entropy(guess, 
remaining_answers)
        ...
        return {"word": best_word, "entropy": 
self.entropy_scores[best_word]}
